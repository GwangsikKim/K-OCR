...
#Image Data Open
with open('data/handwriting_data_info_clean.json','r',encoding='utf-8') as f:
    Korean_data_hand = json.load(f)

with open('data/printed_data_info.json','r',encoding='utf-8') as f:
    Korean_data_print = json.load(f)

with open('data/koreantext.txt','r',encoding='utf-8') as f:
    Korean_2350 = f.read()
...
for i, ID in enumerate(Korean_train_id):
    if i < 28345:
        Image_addr = 'data/1_syllable/' \
            +str(ID)+'.png'
    elif i < 152432:
        Image_addr = 'data/2_syllable/' \
            +str(ID)+'.png'
    else:
        Image_addr = 'data/3_syllable/' \
            +str(ID)+'.png'        
